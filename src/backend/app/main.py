...
@app.get('/app_data/')
async def app_data(url: str) -> AppData:
    print('got here?')
    print(url)
    ...
    if app_id is None:
        raise HTTPException(status_code=404, detail='Could not extract app ID')
...
async def request_inference_bertopic(request: Reviews) -> str:
    async with AsyncClient() as client:
        response = await client.get(f'{config["inference"]["bertopic_url"]}/bertopic/',
                                    params={"reviews": request.reviews})
        return response.json()

async def inference_transformers(request: Reviews) -> str:
    async with AsyncClient() as client:
